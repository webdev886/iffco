 ...
 * @since 1.0
 */
class harmonia_Walker_Nav_Menu_Custom_Fields {
	/**
	...
	public function __construct() {
		...
		$this->fields = array(
			'mega_menu'       => esc_html__( 'Mega Menu', 'harmonia' ),
			'mega_menu_width' => esc_html__( 'Mega Menu Width', 'harmonia' ),
			'column'          => esc_html__( 'Column Width', 'harmonia' ),
			'content'         => esc_html__( 'Content', 'harmonia' ),
			'hide_text'       => esc_html__( 'Hide Text', 'harmonia' ),
			'bg_image'        => esc_html__( 'Background Image', 'harmonia' ),
			'bg_h_position'   => esc_html__( 'Horizontal Position', 'harmonia' ),
			'bg_v_position'   => esc_html__( 'Vertical Position', 'harmonia' ),
			'bg_repeat'       => esc_html__( 'Repeat', 'harmonia' ),
			'bg_size'         => esc_html__( 'Size', 'harmonia' ),
		);

		add_action( 'wp_ajax_harmonia_mega_menu', array( $this, 'harmonia_save_mega_menu' ) );
		add_action( 'wp_ajax_nopriv_harmonia_mega_menu', array( $this, 'harmonia_save_mega_menu' ) );

		add_action( 'wp_ajax_harmonia_mega_fields', array( $this, 'harmonia_mega_fields' ) );
		add_action( 'wp_ajax_nopriv_harmonia__mega_fields', array( $this, 'harmonia_mega_fields' ) );
	}
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_enqueue_style( 'harmonia-nav-menus', get_template_directory_uri() . '/css/nav-menus.css', array(), false );
		wp_enqueue_script( 'harmonia-nav-menus', get_template_directory_uri() . '/js/nav-menus.js', array(), false );
		wp_localize_script( 'harmonia-nav-menus', 'harmonia', array(
			'nonce'    => wp_create_nonce( '_harmonia_nonce' ),
			'megamenu' => esc_html__( 'Mega Menu', 'harmonia' ),
			'savemenu' => esc_html__( 'Save Menu', 'harmonia' )
		) );
	}
	 ...
	 *
	 */
	public function harmonia_save_mega_menu() {
		check_ajax_referer( '_harmonia_nonce', 'lpnonce' );
	 ...
	 */
	public function harmonia_mega_fields() {
		check_ajax_referer( '_harmonia_nonce', 'lpnonce' );
		...
		<div class="field-mega-menu-bg_image field-mega-menu field-of-mega-menu">
			...
			<div class="background-preview <?php echo esc_attr( $class_image ); ?>" id="background-preview"
				...
				<button type="button" id="upload-image"
						class="btn-select"><?php esc_html_e( 'Upload Image', 'harmonia' ); ?></button>
				<input type="hidden" id="edit-item-mega-menu-bg_image" value="<?php echo esc_url( $bg_image ); ?>">
				<button type="button" id="remove-image"
						class="btn-select btn-remove"><?php esc_html_e( 'Remove Image', 'harmonia' ); ?></button>
			</div>
		</div>
		...
		<div class="field-mega-menu-bg_content field-mega-menu field-of-mega-menu">
			<p>
				...
				<select id="edit-item-mega-menu-bg_h_position">
					<option
						value="left" <?php selected( 'left', $bg_h_position ); ?>><?php esc_html_e( 'Left', 'harmonia' ) ?></option>
					<option
						value="right" <?php selected( 'right', $bg_h_position ); ?>><?php esc_html_e( 'Right', 'harmonia' ) ?></option>
					<option
						value="center" <?php selected( 'center', $bg_h_position ); ?>><?php esc_html_e( 'Center', 'harmonia' ) ?></option>
				</select>
			</p>
			...
			<p>
				...
				<select id="edit-item-mega-menu-bg_v_position">
					<option
						value="top" <?php selected( 'top', $bg_v_position ); ?>><?php esc_html_e( 'Top', 'harmonia' ) ?></option>
					<option
						value="bottom" <?php selected( 'bottom', $bg_v_position ); ?>><?php esc_html_e( 'Bottom', 'harmonia' ) ?></option>
					<option
						value="center" <?php selected( 'center', $bg_v_position ); ?>><?php esc_html_e( 'Center', 'harmonia' ) ?></option>
				</select>
			</p>
			...
			<p>
				...
				<select id="edit-item-mega-menu-bg_repeat">
					<option
						value="no-repeat" <?php selected( 'no-repeat', $bg_repeat ); ?>><?php esc_html_e( 'No Repeat', 'harmonia' ) ?></option>
					<option
						value="repeat" <?php selected( 'repeat', $bg_repeat ); ?>><?php esc_html_e( 'Repeat', 'harmonia' ) ?></option>
					<option
						value="repeat-x" <?php selected( 'repeat-x', $bg_repeat ); ?>><?php esc_html_e( 'Repeat Horizontally', 'harmonia' ) ?></option>
					<option
						value="repeat-y" <?php selected( 'repeat-y', $bg_repeat ); ?>><?php esc_html_e( 'Repeat Vertically', 'harmonia' ) ?></option>
				</select>
			</p>
			<p>
				...
				<select id="edit-item-mega-menu-bg_size">
					<option
						value="" <?php selected( '', $bg_size ); ?>><?php esc_html_e( 'Normal', 'harmonia' ) ?></option>
					<option
						value="contain" <?php selected( 'contain', $bg_size ); ?>><?php esc_html_e( 'Contain', 'harmonia' ) ?></option>
					<option
						value="cover" <?php selected( 'cover', $bg_size ); ?>><?php esc_html_e( 'Cover', 'harmonia' ) ?></option>
				</select>
			</p>
		...
		<div class="field-mega-content field-mega-menu field-not-mega-menu">
			...
			<span class="description"><?php esc_html_e( 'HTML and Shortcodes are allowed', 'harmonia' ); ?></span>
		</div>
		<?php
		$output = ob_get_clean();
	...
	public function menu_walker( $walker ) {
		$walker = 'harmonia_Walker_Nav_Menu_Edit';
 ...
 * @since 1.0
 */
class harmonia_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
	/**
